...
create or replace live table dsai_fact_transaction_revenue_table as
...
from dsai_revenue_management_pipeline.dsai_fact_transaction_revenue rev
join dsai_revenue_management_pipeline.dsai_dim_location source on source.location_id=rev.source_id
join dsai_revenue_management_pipeline.dsai_dim_location dest on dest.location_id=rev.destination_id
join dsai_revenue_management_pipeline.dsai_dim_customer cust on cust.id=rev.customer_type_id
join dsai_revenue_management_pipeline.dsai_dim_product p on p.product_id=rev.product_type_id
join dsai_revenue_management_pipeline.dsai_dim_flight f on f.flight_id=rev.flight_id
join dsai_revenue_management_pipeline.dsai_dim_price price on price.price_id=rev.price_type_id
join dsai_revenue_management_pipeline.dsai_dim_promotion pro on pro.price_id=rev.promocode_id
join dsai_revenue_management_pipeline.dsai_dim_currency cur on cur.currency_id=rev.currency_id
...
CREATE or replace LIVE TABLE dsai_fact_transaction_revenue_external_table
...
create or replace live table dsai_fact_revenue_table as
select 
