...
# MAGIC - Compare trials in the [MLflow experiment](#mlflow/experiments/594452848822547/s?orderByKey=metrics.%60val_smape%60&orderByAsc=true)
...
input_data_path = input_client.download_artifacts("2cb5c175bbad4961b6b357c3e1cd6177", "data", input_temp_dir)
...
# MAGIC - All the runs are logged under [this MLflow experiment](#mlflow/experiments/594452848822547/s?orderByKey=metrics.%60val_smape%60&orderByAsc=true)
...
df_aggregation = df_aggregation.assign(ts_id=lambda x:x["source"].astype(str)+"-"+x["destination"].astype(str)+"-"+x["customer_type"].astype(str)+"-"+x["product_type"].astype(str)+"-"+x["source_wind"].astype(str)+"-"+x["destination_wind"].astype(str)+"-"+x["source_humidity"].astype(str)+"-"+x["destination_humidity"].astype(str))
...
def prophet_training(history_pd):
  ...
  hyperopt_estim = ProphetHyperoptEstimator(horizon=horizon, frequency_unit=unit, metric="smape",interval_width=0.8,
                   country_holidays=country_holidays, search_space=search_space, num_folds=2, max_eval=1, trial_timeout=7025,
                   random_state=606842771, is_parallel=run_parallel)
...
with mlflow.start_run(experiment_id="594452848822547", run_name="PROPHET") as mlflow_run:
